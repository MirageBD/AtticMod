...
$(BIN_DIR)/alldata.bin: $(BINFILES)
	...
	$(MEGAADDRESS) $(BIN_DIR)/song.mod               08040000
...
$(EXE_DIR)/$(PROJECT).d81: $(EXE_DIR)/boot.prg.addr.mc $(BIN_DIR)/alldata.bin
	$(RM) $@
	$(CC1541) -n "prjname" -i " 2023" -d 19 -v\
	 \
	 -f "$(PROJECT)" -w $(EXE_DIR)/boot.prg.addr.mc \
	 -f "$(PROJECT).ifc" -w $(BIN_DIR)/alldata.bin \
	$@

# -----------------------------------------------------------------------------

run: $(EXE_DIR)/$(PROJECT).d81

ifeq ($(megabuild), 1)
	$(MEGAFTP) -c "put D:\Mega\$(PROJECT)\exe\$(PROJECT).d81 $(PROJECT).d81" -c "quit"
	$(EL) -m ATTICMOD.D81 -r $(EXE_DIR)/boot.prg.addr.mc
ifeq ($(attachdebugger), 1)
	m65dbg --device /dev/ttyS2
endif
else
	cmd.exe /c $(XMEGA65) -autoload -8 $(EXE_DIR)/$(PROJECT).d81
endif
